...
genre = pd.read_csv('Database\ml-100k\\u.genre',sep='|')
genre= genre.values[:,0]
user_info_col = ['id','age','sex','job','zip']
user = pd.read_csv('Database\ml-100k\\u.user',header=None,sep='|',names=user_info_col)
data_train = pd.read_csv('Database\ml-100k\\ua.base',sep='\t',header=None, names=['user_id','movie_id','rate','time']).as_matrix()
data_test = pd.read_csv('Database\ml-100k\\ua.test',sep='\t',header=None, names=['user_id','movie_id','rate','time']).as_matrix()
...
items = pd.read_csv('Database\ml-100k\\u.item',names=item_col,sep='|',encoding='latin-1')
